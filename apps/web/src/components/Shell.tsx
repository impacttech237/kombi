...
const PRIMARY_NAV: NavItem[] = [
  ...
  { code: 'factures', label: 'Factures', short: 'Factures', Icon: IcoFile },
  ...
  return (
    <aside className="hidden md:flex w-60 flex-col bg-[#0a1408] border-r border-[#1e3222] shrink-0">
      ...
      <nav className="flex-1 px-3 py-4 flex flex-col gap-0.5 overflow-y-auto">
        {PRIMARY_NAV.map(({ code, label, Icon, badge }) => (
        ...
        {SECONDARY_NAV.map(({ code, label, Icon }) => (
          <button key={code} onClick={() => onNav(code)}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${active === code ? 'bg-[#b4e033] text-[#0e1c0f]' : 'text-[#6b9165] hover:bg-[#1e3222] hover:text-[#edf5ea]'}`}>
            <Icon />
            <span className="flex-1 text-left">{label}</span>
...
export function BottomNav({ active, onNav, masquer = [] }: {
  ...
  const visiblePrimary = PRIMARY_NAV.filter((n) => !masquer.includes(n.code));
  const menuItems = [...SECONDARY_NAV, ...PRIMARY_NAV.filter((n) => masquer.includes(n.code))]
    .filter((n) => !masquer.includes(n.code) || !SECONDARY_NAV.includes(n));
  const mainItems = visiblePrimary.slice(0, 4);
  const overflowItems = [...visiblePrimary.slice(4), ...SECONDARY_NAV.filter((n) => !masquer.includes(n.code))];
    ...
    <>
      ...
      {showMenu && overflowItems.length > 0 && (
        <div className="md:hidden fixed inset-0 z-30 flex flex-col justify-end">
          ...
          <div className="relative bg-[#162419] rounded-t-3xl overflow-hidden">
            <div className="w-10 h-1 bg-[#2a4230] rounded-full mx-auto mt-3 mb-1" />
            <div className="px-5 pt-3 pb-2 flex items-center justify-between">
              <p className="text-[#6b9165] text-xs font-medium uppercase tracking-wider">Autres modules</p>
            ...
            <div className="px-4 pb-8 space-y-2 mt-1">
              {overflowItems.map(({ code, label, Icon, badge }) => (
                <button key={code} onClick={() => navigate(code)}
                  className={`w-full flex items-center gap-3.5 px-4 py-3.5 rounded-2xl transition-all active:scale-[.98] ${active === code ? 'bg-[#b4e033] text-[#0e1c0f]' : 'bg-[#1e3222] text-[#edf5ea] border border-[#2a4230]'}`}>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${active === code ? 'bg-[#0e1c0f]/15' : 'bg-[#2a4230]'}`}>
                    <Icon />
                  </div>
                  <span className="flex-1 text-left font-medium text-sm">{label}</span>
                  {badge !== undefined && (
                    <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${active === code ? 'bg-[#0e1c0f]/20 text-[#0e1c0f]' : 'bg-[#f87171]/20 text-[#f87171]'}`}>{badge}</span>
                  )}
                  <IcoChevR cls={`w-4 h-4 shrink-0 ${active === code ? 'text-[#0e1c0f]/50' : 'text-[#4a6b4a]'}`} />
                </button>
              ))}
            </div>
          </div>
...
export { PRIMARY_NAV, SECONDARY_NAV };
