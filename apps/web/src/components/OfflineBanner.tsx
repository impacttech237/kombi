...
export function OfflineBanner() {
  ...
  return (
    <div className={`flex items-center gap-2.5 mb-3 px-3.5 py-2.5 rounded-2xl text-sm font-medium border ${horsLigne ? 'bg-[#fbbf24]/8 border-[#fbbf24]/25 text-[#fbbf24]' : 'bg-[#3a9e6e]/8 border-[var(--k-lime)]/25 text-[var(--k-lime)]'}`}>
      ...
      {enLigne && enAttente > 0 && (
        <button onClick={() => void synchroniser()}
          className="bg-[var(--k-surface-soft)] text-[var(--k-ink)] rounded-lg px-3 py-1 text-xs font-medium border border-[var(--k-line)] shrink-0">
          Synchroniser
        </button>
      )}
    </div>
