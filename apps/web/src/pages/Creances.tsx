  ...
  return (
    <div className="-mx-4 -mt-4 md:-mx-8 md:-mt-6 flex-1 flex flex-col overflow-hidden">
      <div className="px-4 md:px-8 pt-4 pb-2 flex items-center gap-2">
        ...
        <h1 className="text-[var(--k-ink)] text-lg font-bold flex-1">On me doit</h1>
      </div>
      ...
      {!chargement && (ventes!.length > 0 || factures!.length > 0) && (
        <div className="px-4 md:px-8 pb-2">
          <div className="bg-[var(--k-surface)] rounded-2xl p-4 text-center">
            <p className="text-[var(--k-faint)] text-xs">Total dû par les clients</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 pb-24 md:pb-8 space-y-2 pt-1">
        {chargement ? (
          ...
          <p className="text-[var(--k-faint)] text-sm text-center py-8">Aucune créance en cours. Personne ne vous doit rien !</p>
        ) : (
          <>
            {ventes!.map((v) => (
    ...
    <div className="bg-[var(--k-surface)] rounded-2xl p-4">
      <div className="flex items-center gap-3">
        <div className="flex-1 min-w-0">
          <p className="text-[var(--k-ink)] font-medium text-sm truncate">{titre}</p>
          <p className="text-[var(--k-faint)] text-xs mt-0.5">{sousTitre}</p>
        </div>
        {piece?.cle && <IcoFile cls="w-4 h-4 text-[var(--k-lime)] shrink-0" />}
        ...
        <button onClick={() => setOuvert(!ouvert)}
          className="bg-[var(--k-surface-soft)] text-[var(--k-lime)] text-xs px-3 py-2 rounded-xl font-medium border border-[var(--k-lime)]/20 shrink-0">
          Encaisser
        </button>
      </div>
      {ouvert && (
