...
export function Comptabilite({ entreprise, vueInitiale }: {
  ...
  return (
    <div className="-mx-4 -mt-4 md:-mx-8 md:-mt-6 flex-1 overflow-y-auto pb-24 md:pb-8">
      <div className="mx-4 md:mx-8 mt-4 bg-[#3a9e6e]/5 border border-[var(--k-lime)]/20 rounded-2xl p-4 flex items-start gap-3">
        <IcoLayers cls="w-4 h-4 text-[var(--k-lime)] shrink-0 mt-0.5" />
        <div>
          <p className="text-[var(--k-lime)] text-sm font-semibold">Généré automatiquement</p>
          <p className="text-[var(--k-muted)] text-xs mt-1 leading-relaxed">
            ...
            requise. Conforme à la norme <strong className="text-[var(--k-lime)]/80">SYSCOHADA</strong>.
          </p>
        </div>
      </div>
      ...
      <div className="px-4 md:px-8 pt-4 pb-2 flex items-center justify-between">
        <div>
          <p className="text-[var(--k-ink)] font-semibold">États financiers</p>
          <p className="text-[var(--k-faint)] text-xs mt-0.5">{entreprise.raison_sociale}</p>
        </div>
        {(voitJournal || voitBudgets) && (
          <div className="flex items-center gap-1 bg-[var(--k-surface-soft)] rounded-xl p-1 border border-[var(--k-line)]">
            <button onClick={() => setVue('etats')}
              className={`text-xs px-2.5 py-1 rounded-lg font-semibold transition-all ${vue === 'etats' ? 'bg-[#3a9e6e] text-white' : 'text-[var(--k-faint)] hover:text-[var(--k-muted)]'}`}>
              États
            </button>
            {voitJournal && (
              <button onClick={() => setVue('journal')}
                className={`text-xs px-2.5 py-1 rounded-lg font-semibold transition-all ${vue === 'journal' ? 'bg-[#60a5fa] text-white' : 'text-[var(--k-faint)] hover:text-[var(--k-muted)]'}`}>
                Journal
              </button>
            )}
            {voitJournal && (
              <button onClick={() => setVue('clotures')}
                className={`text-xs px-2.5 py-1 rounded-lg font-semibold transition-all ${vue === 'clotures' ? 'bg-[#fbbf24] text-white' : 'text-[var(--k-faint)] hover:text-[var(--k-muted)]'}`}>
                Clôtures
              </button>
            )}
            {voitBudgets && (
              <button onClick={() => setVue('budgets')}
                className={`text-xs px-2.5 py-1 rounded-lg font-semibold transition-all ${vue === 'budgets' ? 'bg-[#a78bfa] text-white' : 'text-[var(--k-faint)] hover:text-[var(--k-muted)]'}`}>
                Budgets
              </button>
            )}
          </div>
        ...
        <>
          <div className="mx-4 md:mx-8 mt-2">
            <div className="bg-[var(--k-surface)] rounded-2xl overflow-hidden">
              <div className="px-4 py-3 border-b border-[var(--k-line)] flex items-center justify-between">
                <div>
                  <h3 className="text-[var(--k-ink)] font-semibold text-sm">Compte de résultat</h3>
                  <p className="text-[var(--k-faint)] text-xs mt-0.5">Produits et charges · Classes 6 &amp; 7</p>
                </div>
                <span className={`text-xs px-2 py-1 rounded-full font-semibold ${profit > 0 ? 'bg-[#4ade80]/10 text-[#4ade80]' : profit < 0 ? 'bg-[#f87171]/10 text-[#f87171]' : 'bg-[var(--k-surface-soft)] text-[var(--k-muted)]'}`}>
              ...
              <div className="p-4 space-y-4">
                ...
                <div className={`rounded-xl p-4 flex justify-between items-center ${profit > 0 ? 'bg-[#4ade80]/5 border border-[#4ade80]/15' : 'bg-[#f87171]/5 border border-[#f87171]/15'}`}>
                  <div>
                    <p className="text-[var(--k-ink)] font-semibold text-sm">Résultat net</p>
                    <p className="text-[var(--k-faint)] text-xs mt-0.5">{regimeIgs ? 'Avant IGS' : 'Avant impôt sur les sociétés'}</p>
                  </div>
                  <span className={`font-mono font-bold text-xl ${profit > 0 ? 'text-[var(--k-lime)]' : 'text-[#f87171]'}`}>{fmt(profit)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {regimeIgs && igs?.igs && (
            <div className="mx-4 md:mx-8 mt-3">
              <div className="bg-[var(--k-surface)] rounded-2xl overflow-hidden">
                <div className="px-4 py-3 border-b border-[var(--k-line)] flex items-center justify-between">
                  <div>
                    <h3 className="text-[var(--k-ink)] font-semibold text-sm">Impôt Général Synthétique</h3>
                    <p className="text-[var(--k-faint)] text-xs mt-0.5">Régime micro-entreprise · Cameroun · art. 45 CGI</p>
                  </div>
                  <span className="text-xs px-2 py-1 rounded-full font-semibold bg-[#fbbf24]/10 text-[#fbbf24]">IGS</span>
                </div>
                <div className="p-4 space-y-2">
                  {[
                    { label: 'CA cumulé exercice', value: fmt(igs.caCumule), color: 'text-[var(--k-ink)]' },
                    ...
                    <div key={label} className="flex justify-between items-center py-1.5 border-b border-[var(--k-line)]/50">
                      <span className="text-[var(--k-muted)] text-sm">{label}</span>
                  ...
                  <div className="bg-[#fbbf24]/5 border border-[#fbbf24]/20 rounded-xl p-3.5 flex justify-between items-center mt-2">
                    <div>
                      ...
                      <p className="text-[var(--k-faint)] text-xs mt-0.5">Au plus tard le 15 avril</p>
                    </div>
          ...
          <div className="mx-4 md:mx-8 mt-3">
            <div className="bg-[var(--k-surface)] rounded-2xl overflow-hidden">
              <div className="px-4 py-3 border-b border-[var(--k-line)] flex items-center justify-between">
                <div>
                  <h3 className="text-[var(--k-ink)] font-semibold text-sm">Bilan simplifié</h3>
                  <p className="text-[var(--k-faint)] text-xs mt-0.5">Situation patrimoniale actuelle</p>
                </div>
              ...
              <div className="grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-[var(--k-line)]">
                <div className="p-4">
                  <p className="text-[var(--k-muted)] text-xs font-medium uppercase tracking-wide mb-3">Actif · Ce que possède l'entreprise</p>
                  <LigneListe lignes={etats.bilan.actif} />
                  <div className="flex justify-between items-center pt-2">
                    <span className="text-[var(--k-ink)] font-medium text-sm">Total actif</span>
                    <span className="font-mono text-[var(--k-ink)] font-semibold">{fmt(etats.bilan.totalActif)}</span>
                  </div>
                </div>
                <div className="p-4">
                  <p className="text-[var(--k-muted)] text-xs font-medium uppercase tracking-wide mb-3">Passif · Ce que doit l'entreprise</p>
                  <LigneListe lignes={passifLines} />
                  <div className="flex justify-between items-center pt-2">
                    <span className="text-[var(--k-ink)] font-medium text-sm">Total passif</span>
                    <span className="font-mono text-[var(--k-ink)] font-semibold">{fmt(etats.bilan.totalPassif)}</span>
                  </div>
                </div>
              </div>
            </div>
...
function LigneSection({ titre, lignes, total, couleurMontant, prefixe }: {
  ...
  return (
    <div>
      <p className="text-[var(--k-muted)] text-xs font-medium uppercase tracking-wide mb-2">{titre}</p>
      {lignes.map((l) => (
        <div key={l.numero} className="flex justify-between items-center py-1.5 border-b border-[var(--k-line)]/50">
          <span className="text-[var(--k-muted)] text-sm">{l.libelle}</span>
      ...
      <div className="flex justify-between items-center pt-2">
        <span className="text-[var(--k-ink)] font-medium text-sm">Total {titre.toLowerCase()}</span>
  ...
  return (
    <>
      {lignes.map((l) => (
        <div key={l.numero} className="flex justify-between items-center py-1.5 border-b border-[var(--k-line)]/50">
          <span className="text-[var(--k-muted)] text-sm">{l.libelle}</span>
          <span className="font-mono text-[var(--k-ink)] text-sm">{fmt(l.montant)}</span>
        </div>
      ))}
    </>
  );
...
function ClotureMensuelleTab({ entreprise }: { entreprise: EntrepriseResume }) {
  ...
  return (
    <div className="pt-2">
      <div className="bg-[var(--k-surface)] rounded-2xl p-4 border border-[var(--k-line)] mb-3">
        <p className="text-[var(--k-ink)] text-sm font-medium mb-1">Clôturer un mois</p>
        <p className="text-[var(--k-faint)] text-xs leading-relaxed mb-3">
        ...
        <div className="flex gap-2">
          <input type="month" value={moisSaisi} onChange={(e) => setMoisSaisi(e.target.value)}
            className="flex-1 bg-[var(--k-surface-soft)] text-[var(--k-ink)] rounded-xl px-3 py-2.5 text-sm border border-[var(--k-line)] focus:border-[#fbbf24] focus:outline-none [color-scheme:dark]" />
          <button onClick={cloturer} disabled={charge}
            className="bg-[#fbbf24] text-white rounded-xl px-4 py-2.5 text-sm font-semibold disabled:opacity-40">
            {charge ? '…' : 'Clôturer'}
          </button>
        </div>
      </div>
      ...
      <p className="text-[var(--k-faint)] text-xs font-medium uppercase tracking-wide mb-2">Mois clôturés</p>
      {clotures === null ? (
        <p className="text-[var(--k-faint)] text-sm text-center py-8">Chargement…</p>
      ) : clotures.length === 0 ? (
        <p className="text-[var(--k-faint)] text-sm text-center py-8">Aucun mois clôturé pour l'instant.</p>
      ) : (
        <div className="bg-[var(--k-surface)] rounded-2xl overflow-hidden">
          {clotures.map((c, i) => (
            <div key={c.annee_mois} className={`flex items-center gap-3 px-4 py-3 ${i < clotures.length - 1 ? 'border-b border-[var(--k-line)]' : ''}`}>
              <div className="flex-1 min-w-0">
                <p className="text-[var(--k-ink)] text-sm font-medium">{c.annee_mois}</p>
                <p className="text-[var(--k-faint)] text-xs mt-0.5">Clôturé le {new Date(c.cloture_le).toLocaleDateString('fr-FR')}</p>
              </div>
  ...
  return (
    <div className="pt-2 space-y-3">
      <div className="bg-[var(--k-surface)] rounded-2xl p-4 border border-[var(--k-line)]">
        <p className="text-[var(--k-ink)] text-sm font-medium mb-1">Objectifs du mois — {moisCourant}</p>
        <p className="text-[var(--k-faint)] text-xs leading-relaxed mb-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          <div>
            <label className="text-[var(--k-muted)] text-xs font-medium block mb-1">CA cible (FCFA)</label>
            <input inputMode="numeric" value={caCible} onChange={(e) => setCaCible(e.target.value.replace(/\D/g, ''))}
              className="w-full bg-[var(--k-surface-soft)] text-[var(--k-ink)] rounded-xl px-3 py-2.5 text-sm border border-[var(--k-line)] focus:border-[#a78bfa] focus:outline-none" />
          </div>
          <div>
            <label className="text-[var(--k-muted)] text-xs font-medium block mb-1">Plafond dépenses (FCFA)</label>
            <input inputMode="numeric" value={plafondDepenses} onChange={(e) => setPlafondDepenses(e.target.value.replace(/\D/g, ''))}
              className="w-full bg-[var(--k-surface-soft)] text-[var(--k-ink)] rounded-xl px-3 py-2.5 text-sm border border-[var(--k-line)] focus:border-[#a78bfa] focus:outline-none" />
          </div>
          <div>
            <label className="text-[var(--k-muted)] text-xs font-medium block mb-1">Marge cible (%)</label>
        ...
        <button onClick={enregistrerBudget} disabled={charge}
          className="mt-3 bg-[#a78bfa] text-white rounded-xl px-4 py-2.5 text-sm font-semibold disabled:opacity-40">
      ...
      <div className="bg-[var(--k-surface)] rounded-2xl p-4 border border-[var(--k-line)]">
        <div className="flex items-center justify-between mb-3">
          <p className="text-[var(--k-ink)] text-sm font-medium">Prévision de trésorerie</p>
          <div className="flex bg-[var(--k-surface-soft)] rounded-lg p-0.5 border border-[var(--k-line)]">
            {([30, 60, 90] as const).map((h) => (
              <button key={h} onClick={() => setHorizon(h)}
                className={`text-xs px-2 py-1 rounded-md font-semibold transition-all ${horizon === h ? 'bg-[#a78bfa] text-white' : 'text-[var(--k-faint)]'}`}>
                {h}j
              </button>
            ))}
          </div>
        ...
        {prevision && (
          <div className="space-y-1.5 text-sm">
            <div className="flex justify-between"><span className="text-[var(--k-muted)]">Solde actuel</span><span className="text-[var(--k-ink)] font-mono">{fmt(prevision.soldeActuel)}</span></div>
            <div className="flex justify-between"><span className="text-[var(--k-muted)]">Entrées attendues</span><span className="text-[#4ade80] font-mono">+{fmt(prevision.entreesAttendues)}</span></div>
            <div className="flex justify-between"><span className="text-[var(--k-muted)]">Sorties attendues</span><span className="text-[#f87171] font-mono">−{fmt(prevision.sortiesAttendues)}</span></div>
            <div className={`flex justify-between pt-2 border-t border-[var(--k-line)] font-semibold ${prevision.soldeProjete >= 0 ? 'text-[var(--k-lime)]' : 'text-[#f87171]'}`}>
      ...
      {seuil && (
        <div className="bg-[var(--k-surface)] rounded-2xl p-4 border border-[var(--k-line)]">
          <p className="text-[var(--k-ink)] text-sm font-medium mb-2">Seuil de rentabilité</p>
          {seuil.seuilCaMensuel != null ? (
            <p className="text-[var(--k-faint)] text-xs leading-relaxed">
              Avec un taux de marge de <span className="text-[var(--k-ink)] font-mono">{seuil.margeSurCoutsVariablesPct}%</span> et
              {' '}<span className="text-[var(--k-ink)] font-mono">{fmt(seuil.chargesFixesMensuelles)}</span> de charges fixes/mois,
            ...
            <p className="text-[var(--k-faint)] text-xs">Pas assez de données de vente pour calculer le seuil.</p>
          )}
        </div>
      )}

      <div className="bg-[var(--k-surface)] rounded-2xl p-4 border border-[var(--k-line)]">
        <p className="text-[var(--k-ink)] text-sm font-medium mb-2">Simuler une baisse de ventes</p>
        <div className="flex gap-2">
          <input inputMode="numeric" value={pctBaisse} onChange={(e) => setPctBaisse(e.target.value.replace(/\D/g, ''))}
            className="w-20 bg-[var(--k-surface-soft)] text-[var(--k-ink)] rounded-xl px-3 py-2 text-sm border border-[var(--k-line)] focus:border-[#a78bfa] focus:outline-none" />
          <span className="text-[var(--k-muted)] text-sm self-center">% de baisse</span>
          <button onClick={() => simulerBaisseVentes(entreprise.id, Number(pctBaisse)).then(setSimBaisse).catch(() => {})}
            className="ml-auto bg-[var(--k-surface-soft)] text-[#a78bfa] rounded-xl px-4 py-2 text-sm font-medium border border-[#a78bfa]/20">
            Simuler
          </button>
        </div>
        {simBaisse && (
          <p className="text-[var(--k-faint)] text-xs mt-3 leading-relaxed">
            CA {fmt(simBaisse.caActuel)} → <span className="text-[var(--k-ink)] font-mono">{fmt(simBaisse.caProjete)}</span>,
      ...
      <div className="bg-[var(--k-surface)] rounded-2xl p-4 border border-[var(--k-line)]">
        <p className="text-[var(--k-ink)] text-sm font-medium mb-2">Simuler un recrutement / investissement</p>
        <div className="flex gap-2">
          <input inputMode="numeric" value={coutRecrutement} onChange={(e) => setCoutRecrutement(e.target.value.replace(/\D/g, ''))}
            ...
            className="flex-1 bg-[var(--k-surface-soft)] text-[var(--k-ink)] rounded-xl px-3 py-2 text-sm border border-[var(--k-line)] focus:border-[#a78bfa] focus:outline-none" />
          <button onClick={() => simulerRecrutement(entreprise.id, Number(coutRecrutement || '0')).then(setSimRecrutement).catch(() => {})}
            className="bg-[var(--k-surface-soft)] text-[#a78bfa] rounded-xl px-4 py-2 text-sm font-medium border border-[#a78bfa]/20">
            Simuler
          </button>
        </div>
        {simRecrutement && (
