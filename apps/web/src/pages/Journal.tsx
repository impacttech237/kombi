...
export function Journal({ entreprise }: { entreprise: EntrepriseResume }) {
  ...
  if (!journal) return <p className="text-[var(--k-faint)] text-sm text-center py-8">Chargement…</p>;

  return (
    <div className="space-y-2">
      <div className="bg-[var(--k-surface)] rounded-2xl p-4 flex items-center justify-between">
        <div>
          <p className="text-[var(--k-ink)] font-semibold text-sm">Intégrité de la chaîne</p>
          <p className="text-[var(--k-faint)] text-xs mt-0.5">{journal.integrite.nbLignes} entrée(s)</p>
        </div>
      ...
      {journal.entrees.length === 0 ? (
        <p className="text-[var(--k-faint)] text-sm text-center py-8">Aucune opération journalisée pour l'instant.</p>
      ) : (
        journal.entrees.map((e) => (
          <div key={e.id} className="bg-[var(--k-surface)] rounded-2xl p-4">
            <div className="flex justify-between items-baseline gap-3">
              <p className="text-[var(--k-ink)] font-medium text-sm">{LABEL_ACTION[e.action] ?? e.action}</p>
              <span className="text-[var(--k-faint)] text-xs shrink-0">{new Date(e.ts + 'Z').toLocaleString('fr-FR')}</span>
            </div>
            <p className="text-[var(--k-faint)] text-xs mt-1">{e.role} · {e.utilisateur_id === 'systeme' ? 'système' : e.utilisateur_id}</p>
          </div>
        ))
      )}
    </div>
