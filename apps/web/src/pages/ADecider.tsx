  ...
  return (
    <div className="-mx-4 -mt-4 md:-mx-8 md:-mt-6 flex-1 flex flex-col overflow-hidden">
      <div className="px-4 md:px-8 pt-4 pb-2 flex items-center gap-2">
        ...
        <h1 className="text-lg font-bold flex-1">À décider aujourd'hui</h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 pb-24 md:pb-8 space-y-3 pt-2">
        {problemes === null ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-[var(--k-ink)] text-sm font-medium">Rien d'urgent aujourd'hui</p>
            <p className="text-[var(--k-faint)] text-xs mt-1">Aucun problème prioritaire détecté sur vos données actuelles.</p>
          </div>
        ) : (
          problemes.map((p, i) => {
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-[var(--k-ink)] font-semibold text-sm">{p.probleme}</p>
                      <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${s.texte} ${s.fond} border ${s.bord}`}>{s.label}</span>
                    </div>
                    <p className="text-[var(--k-faint)] text-xs mt-1 leading-relaxed">{p.cause}</p>
                    <p className={`font-mono font-bold text-lg mt-2 ${s.texte}`}>{fmt(p.impactFinancier)}</p>
                    <button onClick={() => onNav(p.actionCible.page)}
                      className="mt-3 bg-[var(--k-surface-soft)] text-[var(--k-ink)] rounded-xl px-3.5 py-2 text-xs font-semibold border border-[var(--k-line)]">
                      {p.actionSuggeree} →
                    </button>
                  </div>
                </div>
